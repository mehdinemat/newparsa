import { Card, Checkbox, Divider, Grid, GridItem, HStack, Text, VStack } from "@chakra-ui/react";
    ...
    <Grid templateColumns='repeat(4, 1fr)' gap={2} height={'auto'} w={'100%'}>
      <GridItem colSpan={4} height={'100%'}>
        <Card as={VStack} height={'100%'} w={'100%'} bgColor={'#F7F7F7'} variant={'unstyled'} flexShrink={0} gap={0} padding={'10px'} borderRadius={'10px'}>
          ...
          <HStack w={'100%'} alignItems={'start'}>
            <HStack w={'100%'}>
              <Text fontWeight={'500'} fontSize={'14px'} color={'#3646B3'}>{moment(data?.created_at).format("jYYYY/jMM/jDD")}
              </Text>
              <Text fontWeight={'500'} fontSize={'14px'} color={'#3646B3'}>{moment(data?.created_at).format("hh:mm:ss")}
              </Text>
            ...
            <HStack>
              <HStack gap={'14px'} color={'#3646B3'}>
                ...
                <HStack gap={'2px'}>
                  ...
                  <Text fontWeight={'600'} fontSize={'12px'} whiteSpace="nowrap">{data?.viewer} بازدید</Text>
                </HStack>
                <HStack gap={'2px'}>

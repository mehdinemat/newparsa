...
export default function ChatBot() {
  ...
  const {
    ...
    mutate,
  } = useSWR(chatSession && `user/chat/${chatSession}`, null, {
  ...
  const handleOpenChatBot = async () => {
    ...
    // Only trigger a new session if there isn’t one already
    if (!chatSession) {
      ...
      <AnimatePresence mode="sync">
        {isOpen && (
          <MotionBox
            ...
            boxShadow="0px 7px 11.2px 0px #00000026"
            key="chat-box"
            initial={{ opacity: 0, scale: 0.0, y: 300  }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            exit={{ opacity: 0, scale: 0.0, y: 300 , x:200}}
            transition={{ duration: 0.3, ease: "easeOut" }}
          >
            {/* Header */}
